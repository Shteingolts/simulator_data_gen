...
class LJSimulationStandalone:
    """Defines the initial lammps calculation which is used to generate a disordered network.
    Default values work 100%, anything you change may not. The length of `atom_sizes` array
    needs to be equal to `n_atom_types`.
    ...
    def __init__(
        ...
        n_atom_types: int = 4,
        atom_sizes: list[float] = [1.6, 1.4, 1.2, 1.0],
        box_dim: list[float] = [-30.0, 30.0, -30.0, 30.0, -0.1, 0.1],
        inner_block: list[float] = [-10.0, 10.0, -10.0, 10.0, -0.1, 0.1],
        temperature_range: TemperatureRange = TemperatureRange(),
        Kn: float = 1.0,
        n_steps: int = 30000,
        n_parts: list[int] | None = None
    ...
    def write_to_file(self, directory: str | None = None):
        ...
        if self.n_parts is None:
            for i in range(self.n_atom_types):
                create_atoms_section += f"create_atoms {i+1} random ${{npart}} {random.randint(1, 999999)} inner overlap 0.5 maxtry 100\n"
                group_section += f"group size{i+1} type {i+1}\n"
                group_size_section += f"set group size{i+1} diameter {self.atom_sizes[i]}\n"
        else:
            for i, n in enumerate(self.n_parts):
                create_atoms_section += f"create_atoms {i+1} random {n} {random.randint(1, 999999)} inner overlap 0.5 maxtry 100\n"
                group_section += f"group size{i+1} type {i+1}\n"
                group_size_section += f"set group size{i+1} diameter {self.atom_sizes[i]}\n"
...
# create 2D box
region box block {self.box_dim[0]} {self.box_dim[1]} {self.box_dim[2]} {self.box_dim[3]} {self.box_dim[4]} {self.box_dim[5]}
region inner block {self.inner_block[0]} {self.inner_block[1]} {self.inner_block[2]} {self.inner_block[3]} {self.inner_block[4]} {self.inner_block[5]}
create_box {self.n_atom_types} box
...
reset_timestep 0
fix 9  all langevin {self.temperature_range.T_start} {self.temperature_range.T_end} {self.temperature_range.bias} {random.randint(1, 999999)}
fix 10 all nve/sphere disc


# output time step, temperature, average kinetic and potential energy
# thermo_style custom step temp ke pe
# thermo		100
dump            1 all custom 50 dump.lammpstrj id x y z vx vy vz
